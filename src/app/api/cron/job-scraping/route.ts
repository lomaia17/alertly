...
import { getUserPreferences, saveNewJobsForUser, getSavedJobsForUser, Job } from '@/lib/fireBaseConfig';

const resend = new Resend(process.env.RESEND_API_KEY);

// Reuse your existing types and helper functions from your route.ts
type JobA = {
  jobTitle: string;
  company: string;
  published?: string;
...
onst extractJobIdJobsGe = (link: string): string | null => {
...
export async function GET() {
  try {
    // Fetch all user preferences from Firebase
    ...
    const scrapedJobs = await scrapeJobs(userPreferences);
      ...
      for (const preference of userPreferences) {
        ...
        if (newJobs.length > 0) {
          ...
          const mapJobAtoJob = (jobA: JobA): Job => ({
            ...
            title: jobA.jobTitle.trim(),
            jobTitle: jobA.jobTitle.trim(),
            link: jobA.link,
            source: jobA.source,
            company: jobA.company,
            location: jobA.location || 'Unknown',
            description: `${jobA.company} is hiring for ${jobA.jobTitle}.`,
            keywords: jobA.jobTitle
              .toLowerCase()
              .split(' ')
              .filter((k) => k.length > 0),
          });
    ...
    return NextResponse.json(
      { message: 'Cron job executed successfully', jobsFound: allScrapedJobs.length },
      { status: 200 }
    );
  } catch (error) {
    console.error('Error in cron job:', error);
    return NextResponse.json(
      { message: 'Error executing cron job', error: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}